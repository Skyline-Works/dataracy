...
export const metadata = {
    title: "Community | Dataracy",
    description: "Join the conversation and share insights.",
};
...
export default async function CommunityPage({
    ...
    // Community often doesn't have images, so we might want a different layout or default image
    const getPostImage = (post: any) => {
        if (post.image_url) return post.image_url;
        return '/images/defaults/study_cover.png';
    }
            ...
            <main className="flex-1 pt-32 pb-20 container mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside className="lg:col-span-3 hidden lg:flex flex-col gap-6">
                    <div className="sticky top-28 space-y-6">
                        <Link href="/write" className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-gradient-to-r from-fuchsia-600 to-pink-600 text-white font-bold hover:shadow-lg hover:shadow-fuchsia-500/25 transition-all text-sm group">
                            <PenLine className="w-4 h-4 group-hover:scale-110 transition-transform" /> 커뮤니티 글쓰기
                        </Link>

                        <div className="glass-card p-4 rounded-2xl">
                            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-2">Community</h3>
                            <p className="text-sm text-slate-400 px-2">
                                데이터 분석가들과 자유롭게 소통하고 스터디원을 모집해보세요.
                            </p>
                        </div>
                ...
                <div className="lg:col-span-9 flex flex-col gap-8 w-full min-w-0">
                    ...
                    {(!posts || posts.length === 0) ? (
                        <div className="glass-card p-12 rounded-3xl text-center text-slate-400 border border-dashed border-white/10 flex flex-col items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-white mb-1">등록된 글이 없습니다</h3>
                                <p className="text-sm">커뮤니티의 첫 번째 주인공이 되어보세요.</p>
                            </div>
                            <Link href="/write" className="mt-2 px-6 py-2.5 rounded-full bg-fuchsia-600 text-white text-sm font-bold hover:bg-fuchsia-500 transition-colors">
                                글 작성하기
                            </Link>
                        </div>
                    ) : (
                            ...
                            {posts.map((post: any) => (
                                <Link href={`/posts/${post.id}`} key={post.id} className="block group w-full">
                                    <div className="glass-card p-5 rounded-2xl flex flex-col gap-4 group-hover:bg-white/5 transition-all h-full cursor-pointer w-full overflow-hidden border border-white/5">
                                        ...
                                        {/* Optional: Show Image for Community/Study as well or different layout */}
                                        <div className="rounded-xl bg-slate-800/50 mb-1 relative overflow-hidden border border-white/5 flex items-center justify-center w-full h-40">
                                            ...
                                            <span className={`absolute top-3 right-3 backdrop-blur text-xs px-3 py-1.5 rounded-lg text-white font-bold z-10 shadow-lg border border-white/10 ${post.type === 'STUDY' ? 'bg-pink-600/80' : 'bg-fuchsia-600/80'}`}>
                                                {post.type}
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex flex-col flex-1">
                                            <h3 className="font-bold text-white text-lg group-hover:text-fuchsia-300 transition-colors w-full mb-2 leading-snug line-clamp-2">
                                                {post.title}
                                            </h3>
