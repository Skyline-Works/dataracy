...
import { Database, PenLine, Download } from "lucide-react";
import Navbar from "@/components/Navbar";
import SidebarFilter from "@/components/SidebarFilter";
import { createClient } from "@/lib/supabase/server";

export const metadata = {
    title: "Datasets | Dataracy",
    description: "Explore high-quality datasets for your projects.",
};
...
export default async function DatasetsPage({
    ...
    const getPostImage = (post: any) => {
        ...
        return '/images/defaults/data_cover.png'; // Default for Datasets
    }
            ...
            <main className="flex-1 pt-32 pb-20 container mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside className="lg:col-span-3 hidden lg:flex flex-col gap-6">
                    <div className="sticky top-28 space-y-6">
                        <Link href="/write" className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-bold hover:shadow-lg hover:shadow-violet-500/25 transition-all text-sm group">
                            <PenLine className="w-4 h-4 group-hover:scale-110 transition-transform" /> 새 데이터셋 등록
                        </Link>

                        <div className="glass-card p-4 rounded-2xl">
                            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-2">Datasets</h3>
                            <p className="text-sm text-slate-400 px-2">
                                프로젝트에 바로 활용할 수 있는 정제된 고품질 데이터셋을 찾아보세요.
                            </p>
                        </div>
                ...
                <div className="lg:col-span-9 flex flex-col gap-8 w-full min-w-0">
                    ...
                    {(!posts || posts.length === 0) ? (
                        <div className="glass-card p-12 rounded-3xl text-center text-slate-400 border border-dashed border-white/10 flex flex-col items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-white mb-1">등록된 데이터셋이 없습니다</h3>
                                <p className="text-sm">가치 있는 데이터를 공유해보세요.</p>
                            </div>
                            <Link href="/write" className="mt-2 px-6 py-2.5 rounded-full bg-violet-600 text-white text-sm font-bold hover:bg-violet-500 transition-colors">
                                데이터셋 등록하기
                            </Link>
                        </div>
                    ) : (
                                ...
                                <Link href={`/posts/${post.id}`} key={post.id} className="block group w-full">
                                    <div className="glass-card p-5 rounded-2xl flex flex-col gap-4 group-hover:bg-white/5 transition-all h-full cursor-pointer w-full overflow-hidden border border-white/5">
                                        <div className="rounded-xl bg-slate-800/50 mb-1 relative overflow-hidden border border-white/5 flex items-center justify-center w-full h-48">
                                            ...
                                            <span className="absolute top-3 right-3 backdrop-blur bg-violet-600/80 text-xs px-3 py-1.5 rounded-lg text-white font-bold z-10 shadow-lg border border-white/10">
                                                DATA
                                            </span>
                                        </div>

                                        <div className="flex flex-col flex-1">
                                            <h3 className="font-bold text-white text-lg group-hover:text-violet-300 transition-colors w-full mb-2 leading-snug line-clamp-2">
                                                {post.title}
                                            </h3>
